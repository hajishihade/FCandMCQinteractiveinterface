...
// Instance methods
MCQSeriesSchema.methods.getNextSessionId = function() {
  return this.sessions.length > 0
    ? Math.max(...this.sessions.map(s => s.sessionId)) + 1
    : 1;
};

MCQSeriesSchema.methods.getActiveSession = function() {
  return this.sessions.find(session => session.status === 'active') || null;
};

MCQSeriesSchema.methods.getCompletedSessions = function() {
  return this.sessions.filter(session => session.status === 'completed');
};

MCQSeriesSchema.methods.getTotalQuestions = function() {
  return this.sessions.reduce((total, session) => total + session.questions.length, 0);
};

MCQSeriesSchema.methods.getTotalCorrect = function() {
...
MCQSeriesSchema.methods.getSuccessRate = function() {
  const total = this.getTotalQuestions();
  if (total === 0) return 0;
  return Math.round((this.getTotalCorrect() / total) * 100);
};

export default mongoose.model('MCQSeries', MCQSeriesSchema);